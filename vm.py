...
def v(arg):
    ...
    block_y = vm.init_state(
        addr=contract, exec_env_id=y, msg_sender=ZeroExt(96, BitVec("address1", 160))
    )
    ...
    with open(name + ".txt", "w") as f:
        f.write(
            str(vm.vulnerability_verifier.independently_executed_state)
            + "\n"
            + str(vm.vulnerability_verifier.cross_called_executed_state)
        )
    ...
    vm.cfmanager = ControlFlowManager()
    vm.init_state(
        addr=contract, exec_env_id=x, msg_sender=ZeroExt(96, BitVec("address1", 160))
    )
    vm.get_exec_env().get_msg_data().set_function_id(x)
    vm.get_exec_env().get_msg_data().set_arguments(1000)
    vm.run()
    ...
    return (
        hex(x.as_long()),
        hex(y.as_long()),
        vm.vulnerability_verifier.diff_states(),
        s,
    )
...
def v2(arg):
    ...
    block_y = vm.init_state(addr=contract, exec_env_id=y, msg_sender=msg_sender)
    vm.get_exec_env().get_msg_data().set_function_id(y)
    # vm.get_exec_env().get_msg_data().set_arguments(1000)
    vm.get_exec_env().get_msg_data().set_concrete_arguments(BitVecVal(0xFF, 256))
    vm.cfmanager.basic_blocks.pop()
    block_x = vm.init_state(
        addr=contract, exec_env_id=x, depth_of_call=2, msg_sender=msg_sender
    )
    block_x.block_number = 1
    vm.cfmanager.basic_blocks.append(block_y)
    vm.get_exec_env().get_msg_data().set_function_id(x)
    ...
    vm.vulnerability_verifier.set_executing_cross_function()
    vm.cfmanager = ControlFlowManager()
    vm.init_state(addr=contract, exec_env_id=x, msg_sender=msg_sender)
    vm.get_exec_env().get_msg_data().set_function_id(x)
    # vm.get_exec_env().get_msg_data().set_arguments(1000)
    vm.get_exec_env().get_msg_data().set_concrete_arguments(BitVecVal(0xFF, 256))
    vm.run()
...
class VM:
    ...
    def verify_full_state(self):
        ...
        while self.primary_contract_index < 1:
            ...
            print(
                "callable fids=",
                self.vulnerability_verifier.get_callable_function_ids(),
            )
            ...
            result = single(
                ...
                [
                    id.as_long()
                    for id in self.vulnerability_verifier.callable_function_ids
                ],
                [id.as_long() for id in self.vulnerability_verifier.function_ids],
                str(self.get_exec_env().this_code),
            )
    ...
    def verify_full_state_create(self):
        ...
        while self.primary_contract_index < 1:
            ...
            msg_sender = BitVecVal(0x1111111111111111111111111111111111111111, 256)
            self.init_state(
                contract, msg_sender=msg_sender,
            )
            # self.get_machine_state().set_balance(BitVecVal(0xff, 256))

            self.get_exec_env().get_msg_data().set_function_id()
            self.get_exec_env().get_msg_data().set_concrete_arguments(
                BitVecVal(0xFF, 256)
            )
            # self.get_exec_env().get_msg_data().set_arguments(1000)
            self.run()
    ...
    def verify_create_based_re_entrancy(self, verifier=None):
        ...
        while self.primary_contract_index < 1:
            ...
            for x in self.vulnerability_verifier.callable_function_ids:
                ...
                self.cfmanager = ControlFlowManager()
                self.init_state(
                    addr=contract, exec_env_id=x.as_long(), msg_sender=msg_sender
                )
                ...
                for y in self.vulnerability_verifier.function_ids:
                    ...
                    self.init_state(
                        addr=contract,
                        exec_env_id=y.as_long(),
                        storage=caller_state,
                        balance=balance,
                        msg_sender=msg_sender,
                    )
                    ...
                    with open(name + ".txt", "w") as f:
                        f.write(
                            str(
                                self.vulnerability_verifier.independently_executed_state
                            )
                            + "\n"
                            + str(
    ...
    def check_jumpdest(self, dest: BitVecNumRef):
        d = dest.as_long()
        return (
            True if self.get_exec_env().this_code[d * 2 : d * 2 + 2] == "5b" else False
        )
    ...
    def branch(self, continuable, jumpable, condition):
        return self.cfmanager.inherit_from_processing_block(
            continuable, jumpable, condition
        )

    def external_call(self, exec_env: ExecutionEnvironment):
    ...
    def run(self):
        while True:
            ...
            self.cfmanager.visited_address[self.get_processing_block().get_path()][
                self.get_pc()
            ] = True

            if (
                self.get_machine_state().get_pc()
                >= len(self.get_exec_env().this_code) // 2
            ):
            ...
            s = []
            self.cfmanager.processing_block.get_machine_state().set_gas(
                simplify(
    ...
    def terminate(self):
        ...
        self.get_processing_block().add_constraint_to_path_condition(
            self.get_balance() >= 0
        )
    ...
    def op_stop(self, s):
        ...
        if self.cfmanager.rollback_from_call_stack(self.vulnerability_verifier):
            ...
            if (
                self.get_processing_block()
                .get_exec_env()
                .get_code()[prevpc * 2 : prevpc * 2 + 2]
                == "f0"
            ):
    ...
    def op_sdiv(self, s):
        if isinstance(s[1], BitVecNumRef) and s[1].as_singed_long() == 0:
            ...
            if s[0].as_signed_long() == -(2 ** 255) and s[1].as_signed_long() == -1:
                ...
                self.push_to_stack(
                    BitVecVal256(s[0].as_signed_long() // s[1].as_singed_long())
                )
        else:
    ...
    def op_lt(self, s):
        self.push_to_stack(
            If(BV2Int(s[0]) < BV2Int(s[1]), BitVecOne256(), BitVecZero256())
        )
        self.increment_pc()
    ...
    def op_slt(self, s):
        self.push_to_stack(
            If(
                bv_to_signed_int(s[0]) < bv_to_signed_int(s[1]),
                BitVecOne256(),
                BitVecZero256(),
            )
        )
    ...
    def op_sha3(self, s):
        ...
        if isinstance(s[0], BitVecNumRef) and isinstance(s[1], BitVecNumRef):
            ...
            for i in range(offset + 1, offset + length):
                ...
                data = Concat(
                    data, self.get_machine_state().get_memory().get_one_byte(i)
                )
        k = keccak_256()
    ...
    def op_calldatacopy(self, s):
        if not (
            isinstance(s[0], BitVecNumRef)
            and isinstance(s[1], BitVecNumRef)
            and isinstance(s[2], BitVecNumRef)
        ):
            raise DevelopmentErorr()
    ...
    def op_codecopy(self, s):
        ...
        tmpcode = self.get_code()[codeoffset : codeoffset + copysize] + "00" * (
            codeoffset + copysize - codesize
        )
        ...
        for i in range(copysize // 2):
            memory.mstore8(
                memoffset + i, BitVecVal256(int(tmpcode[i * 2 : i * 2 + 2], 16))
            )
    ...
    def op_extcodesize(self, s):
        ...
        try:
            self.push_to_stack(
                BitVecVal256(len(self.σ.get_account(addr).get_bytecode()) // 2)
            )
        except:
            # TODO
    ...
    def op_returndatasize(self, s):
        self.push_to_stack(
            BitVecVal256(self.get_machine_state().get_return_data().size())
        )
        self.increment_pc()
    ...
    def op_blockhash(self, s):
        n = BV2Int(s[0])
        current_block_number = BV2Int(
            self.get_exec_env().get_block_header().get_block_number()
        )
    ...
    def op_jump(self, s):
        if not isinstance(s[0], BitVecNumRef):
            raise DevelopmentErorr(
                "cannot to jump to address expressed with symbol variable"
            )
        # 現状ではrun中にjumpdestかを確認
    ...
    def op_jumpi(self, s):
        ...
        # ジャンプ可能か
        jumpable = solve_and_time(
            And(condition, self.get_processing_block().get_path_condition())
        )
        # 継続可能か
        continuable = solve_and_time(
            And(Not(condition), self.get_processing_block().get_path_condition())
        )
        # print('op_jumpi')
    ...
    def op_create(self, s):
        ...
        for i in range(length):
            ...
            if (not isinstance(op, BitVecNumRef)) and (
                not isinstance(simplify(op), BitVecNumRef)
            ):
                print(op)
        ...
        exec_env = ExecutionEnvironment(
            Ia=BitVecZero256(), Ib=new_code, Is=self.get_address(), Iv=s[0]
        )
        # call initialization code
        self.external_call(exec_env=exec_env)  # self.get_account_num(),

    def op_call(self, s):

        if not (
            isinstance(s[3], BitVecNumRef)
            ...
            print(s, self.get_pc())
            raise DevelopmentErorr(
                "some parameter given to CALL must be concrete value"
            )
        ...
        if self.vulnerability_verifier.is_first_call():
            ...
            returndata = [
                BitVec(
                    "Return_data{}-{}_{}".format(
                        self.get_exec_env().get_exec_env_id(), self.get_pc(), i
                    ),
                    8,
                )
            ...
            for i in range(retLength):
                self.get_machine_state().get_memory().mstore8(
                    i + retOffset, returndata[i]
                )
            self.get_machine_state().set_return_data(
                Returndata(immediate_data=returndata)
            )
    ...
    def op_return(self, s):
        ...
        for i in range(offset, offset + length):
            returndata.append(self.get_machine_state().get_memory().get_one_byte(i))

        next_block = self.cfmanager.rollback_from_call_stack(
            self.vulnerability_verifier
        )
        if next_block:

            self.get_machine_state().set_return_data(
                Returndata(
                    block_number=next_block.get_block_number(),
                    immediate_data=returndata,
                )
            )
            for i in range(retLength):
                next_block.get_machine_state().get_memory().mstore8(
                    retOffset + i, returndata[i]
                )
            ...
            if (
                ...
                addr = self.add_primary_contract(
                    "".join([format(v.as_long(), "02x") for v in returndata])
                )

                # TODO gas calculation
                condition = And(
                    s[0] <= self.get_balance(),
                    self.get_machine_state().get_stack().get_stack_size() < 1024,
                )
                self.push_to_stack(simplify(If(condition, addr, BitVecZero256())))
            elif (
                self.get_processing_block()
                .get_exec_env()
                .get_code()[prevpc * 2 : prevpc * 2 + 2]
